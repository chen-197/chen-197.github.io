...
updated: 2026-09-16 10:00     # 修改时间，后台保存时自动写入；新建时等于创建时间
tags: [nodejs, 架构]          # 多个标签
categories: 技术               # 一个或多个
...
文章页和首页卡片会显示「发表于 X · 更新于 Y」（修改时间比创建时间晚超过一天才显示"更新于"）；**文章列表按修改时间倒序排列**。

修改时间**直接存在 front-matter 的 `updated` 里**，不依赖 git 提交时间或文件时间——所以跨机器、CI 上结果一致，一次提交多篇文章也不会让它们拿到同一个时间。规则：

- 后台新建文章：`updated` = `date`
- 后台保存已有文章：`updated` = 保存那一刻的时间（想让文章排到最前，改完保存即可）
- 「更新于」一栏手填了时间：用你填的值（留空就是上面两条的默认行为）
- 手工新增的 .md 没写 `updated`：视为创建后没改过，等于创建时间
